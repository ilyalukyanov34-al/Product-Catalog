...
function CustomDropdown<T extends string>({
  ...
  // Отдельный эффект — закрытие при скролле страницы.
  // Сделан отдельным useEffect, а не объединён с предыдущим,
  // чтобы каждый эффект отвечал ровно за одно действие.
  useEffect(() => {
    if (!open) return;
    ...
    // passive: true — подсказка браузеру, что обработчик не блокирует
    // скролл, благодаря чему сам скролл остаётся плавным
